use gmrag_api::auth::outbox::AuthzOutboxProcessorConfig;
use gmrag_api::auth::outbox_health::{
    AuthzOutboxHealthOptions, DEFAULT_MAX_AGE_MINUTES, DEFAULT_MAX_FAILED,
    check_authz_outbox_health,
};
...
async fn main() {
    ...
    let max_retries = AuthzOutboxProcessorConfig::from_env().max_retries;
    ...
    let options = AuthzOutboxHealthOptions {
        max_failed: args.max_failed,
        max_age_minutes: args.max_age_minutes,
        max_retries,
    };
    let report = match check_authz_outbox_health(&pool, options).await {
        Ok(report) => report,
        Err(error) => {
            eprintln!("failed to query authz_outbox health: {error}");
            std::process::exit(2);
        }
    };
...
fn print_usage() {
    println!(
        "check-authz-outbox-health\n\n\
Usage:\n  \
  cargo run --bin check-authz-outbox-health -- [--max-failed N] [--max-age-minutes N] [--json]\n\n\
Options:\n  \
...
Read-only synthetic alert health-check. It never mutates authz_outbox.\n\n\
Exit codes:\n  \
...
mod tests {
    ...
    #[test]
    fn defaults_match_ops004_contract() {
        assert_eq!(
